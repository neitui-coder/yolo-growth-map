...
function pickExtraNodeFields(node) {
  ...
  Object.keys(node || {}).forEach(function (key) {
    if (['date', 'type', 'desc', 'images', '_idx'].indexOf(key) === -1) {
      extra[key] = node[key];
    }
  });
  return extra;
...
Page({
  ...
  onLoad: function (options) {
    ...
    if (!(app.canManageUser && app.canManageUser(userId))) {
      wx.showToast({ title: '仅本人或运营团队可编辑成长地图', icon: 'none' });
      wx.navigateBack();
      return;
    }
    ...
    if (isEdit) {
      ...
      this.setData({
        ...
        extraNodeData: pickExtraNodeFields(node)
      });
      wx.setNavigationBarTitle({ title: '编辑成长节点' });
    } else {
      this.setData({ extraNodeData: {} });
